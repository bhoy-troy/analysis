...
fig = px.histogram(cabinet_df, x='temperature_celsius', nbins=50,
                   title=f"Temperature Distribution - {selected_cabinet}")
fig.update_layout(xaxis_title="Temperature (°C)", yaxis_title="Count")
st.plotly_chart(fig, use_container_width=True)

# Daily temperature summary
st.subheader("Daily Statistics")
daily_stats = cabinet_df.groupby('date')['temperature_celsius'].agg([
...
st.download_button(
    ...
    data=csv,
    file_name=f"{selected_cabinet}_daily_stats.csv",
    mime="text/csv"
)
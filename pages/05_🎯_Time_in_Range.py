...
with col1:
    st.metric(
        "Optimal Range",
        f"{time_range_metrics['optimal_pct']:.1f}%",
        help=time_range_metrics['optimal_range']
    )
...
with col2:
    st.metric(
        "Warning Range",
        f"{time_range_metrics['warning_pct']:.1f}%",
        help=time_range_metrics['warning_range']
    )
...
with col4:
    st.metric(
        "Below Optimal",
        f"{time_range_metrics['below_optimal_pct']:.1f}%"
    )
    st.caption("Too cold")
...
cabinet_df_daily['in_optimal'] = (
    (cabinet_df_daily['temperature_celsius'] >= (
        -25 if is_freezer else 0)) &
    (cabinet_df_daily['temperature_celsius'] <= (
        -18 if is_freezer else 5))
)

daily_compliance = cabinet_df_daily.groupby('date').agg({
    'in_optimal': lambda x: (x.sum() / len(x)) * 100
}).reset_index()
...
st.download_button(
    ...
    data=csv,
    file_name=f"{selected_cabinet}_compliance.csv",
    mime="text/csv"
)
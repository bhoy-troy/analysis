...
c1.metric("Peak Demand (kW)",    f"{peak_kw:.1f}" if peak_kw else "N/A",
...
c3.metric("Avg Power Factor",    f"{avg_pf:.3f}" if avg_pf else "N/A",
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "Energy Overview", "Power Factor", "Peak Demand", "Load Heatmap", "Raw Data"
])

with tab1:
    st.subheader("Energy profile — kW, kVA, kVAr & Max Demand")
    fig = go.Figure()
    for col, dash in [("Total kW","solid"),("Total kVA","solid"),
                      ("Total kVAr","solid"),("Max Demand kW","dash")]:
        if col in df.columns:
            fig.add_trace(go.Scatter(x=df["timestamp"], y=df[col], name=col,
                                     mode="lines", line=dict(width=2, dash=dash)))
    fig.add_hline(y=MIC, line_dash="dot", line_color="red", line_width=2,
                  annotation_text=f"MIC {MIC} kW", annotation_position="top left")
    fig.add_hline(y=MIC * demand_warning_pct / 100, line_dash="dot", line_color="orange",
                  annotation_text=f"Warning {demand_warning_pct}%", annotation_position="bottom right")
    fig.update_layout(
...
with tab2:
    if "Average PF" in df.columns:
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=df["timestamp"], y=df["Average PF"],
                                  mode="lines", fill="tozeroy", name="Avg PF", line=dict(width=2)))
        fig2.add_hline(y=pf_target, line_dash="dot", line_color="orange",
                       annotation_text=f"Target {pf_target}", annotation_position="top left")
        fig2.update_layout(
            xaxis=dict(title="Timestamp"),
        ...
        fig_hist.add_vline(x=pf_target, line_dash="dot", line_color="orange",
...
with tab3:
    if "Total kW" in df.columns:
        ...
        fig3 = px.bar(hourly_peak, x="Hour", y="Peak kW", color="Status",
        ...
        fig3.add_hline(y=MIC * demand_warning_pct / 100, line_dash="dot", line_color="orange",
        ...
        fig4 = go.Figure()
        fig4.add_trace(go.Scatter(x=df["timestamp"], y=df["MIC %"],
                                  mode="lines", fill="tozeroy", name="Demand % MIC",
                                  line=dict(width=2)))
        fig4.add_hline(y=100, line_dash="dot", line_color="red", annotation_text="MIC 100%")
        fig4.add_hline(y=demand_warning_pct, line_dash="dot", line_color="orange",
                       annotation_text=f"Warning {demand_warning_pct}%")
        fig4.update_layout(
            xaxis=dict(title="Timestamp"),
...
with tab4:
    if "Total kW" in df.columns:
        ...
        if pivot.shape[0] >= 2:
            fig5 = px.imshow(pivot, aspect="auto", color_continuous_scale="RdYlGn_r",
